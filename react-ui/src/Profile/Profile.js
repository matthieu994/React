...
const DEFAULT_IMG = "https://d1nhio0ox7pgb.cloudfront.net/_img/o_collection_png/green_dark_grey/256x256/plain/user.png"
...
export default class Profile extends Component {
    ...
    getFriends() {
        axios.get('/Profile/friends', this.getHeaders()
            ...
            res.data.sort(function (a, b) { return (a._id > b._id) ? 1 : ((b._id > a._id) ? -1 : 0) });
            this.setState({
                friends: res.data
            })
        }).catch(err => console.log(err))
        ...
        return this.state.friends.map((friend, index) => {
            ...
            return (
                <div className={"user-card " + friend.status} key={index}>
                    <img className="user-img" src={friend.url ? friend.url : DEFAULT_IMG} alt="Profile" />
            ...
            return (
                <div className={item.done ? "list-group-item disabled pt-3" : "list-group-item pt-3 list-group-item-action list-group-item-info"} key={item._id}>
                    ...
                    <i className="fas fa-share-square"
                        onClick={() => this.sendTodo(item._id)}></i>
                </div>
            );
        });
